...
<head>
    ...
    <meta name="description" content="Admin Dashboard for Home Management" />
    <title>Home Management - Admin Ican</title>
...
<body class="sb-nav-fixed">
    ...
    <a class="navbar-brand ps-3" href="/dashboard">Manage Homes</a>
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    ...
                    <div class="row">
                        <a href="{{ route('admin.home.create') }}" class="btn btn-primary create-btn" role="button">
                            <i class="fas fa-plus-circle"></i> Create New Home
                        </a>
                    </div>
                    ...
                    <table class="table table-hover table-striped" id="myTable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($row->image)
                                    <img src="{{ asset('storage/' . $row->image) }}" alt="Home Image" width="50" height="50" style="object-fit: cover; border-radius: 5px;">
                                    @else
                                    <span>No Image</span>
                                    @endif
                                </td>
                                <td>
                                    <a href="{{ route('admin.home.show', $row) }}" class="btn btn-sm btn-info">Detail</a>
                                    <a href="{{ route('admin.home.edit', $row) }}" class="btn btn-sm btn-warning">Edit</a>
                                    <button type="button" class="btn delete-btn btn-danger btn-sm" data-id="{{ $row->id }}">Delete</button>
                                    <form action="{{ route('admin.home.destroy', $row->id) }}" id="delete-form-{{ $row->id }}" method="POST" style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
